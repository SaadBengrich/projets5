...
$p = $_SESSION["id"];
$i = Commande::getcbyclientid($p, "commande", $connect->conn);
$i = 1;

if ($i !== null) {
    $purchases = Acheter::selectAllAchats($connect->conn);

    if (is_iterable($purchases)) {
        foreach ($purchases as $purchase) {
            $imageData = Produit::getImageData($connect->conn, $purchase['id_p']);
            $base64Image = base64_encode($imageData);

            echo "
                <tr>
                    <td><img src='data:image/png;base64,{$base64Image}' alt='Product Image' width='50' height='50'></td>
                    <td>{$purchase['qte']}</td>
                    <td>{$purchase['montant']}</td>
                    <td> <a href='deletepro.php?id={$purchase['id_p']}'><i class='fa-solid fa-trash'></i></a></td>

                    
                </tr>";
        }
    } else {
        // Handle the case when $purchases is not iterable (null or not an array/object)
        echo "No data available.";
    }
} else {
    // Handle the case when $i is null
    echo "No command ID available.";
}
?>
